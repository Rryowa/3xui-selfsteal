...
# TCP Keepalive Tuning (Aggressive for VPN/Proxy stability)
net.ipv4.tcp_keepalive_time=300
net.ipv4.tcp_keepalive_probes=9
net.ipv4.tcp_keepalive_intvl=60

# Buffer Tuning (Optimized for long-distance cross-border links)
...
ssh_hardening() {
    ...
    # Apply Hardening via high-priority override (handles Include bug)
...
EOF
    ...
    # Clean up the main config to avoid duplicates and ensure Include is active
...
main() {
    ...
    echo -e "  2. Create 2GB swap space (re-configures swap if exists)"
    echo -e "  3. Configure NTP time synchronization"
    echo -e "  4. Apply system tuning (BBR congestion control, TFO, buffer tuning)"
    echo -e "  5. Optionally configure SSH public-key hardening"
    echo -e "  6. Offer a system reboot option"
    echo -e ""
